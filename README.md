...
Please note, I have only tested this addon on private servers so there may be issues if this is used on retail. However, most of the private servers I have played on use the retail client. The bigger issue is potential problems with content beyond level 75. Please let me know any issue you experience using this addon on retail.<br>
<br>
...
In order to set this up simply add all levels of a spell to the JOB.lua file with the same slot specified, in descending order of importance as seen below. By importance, I do not necessarily mean in order of highest tier to lowest tier of the same family of spell. This should still work fine with any combination of spells in any order.
 
![Tiered Spells](images/readme/TieredSpells.png)

This works seamlessly with the level sync feature above as well. For example, syncing down to level 30 will turn Dia II back into Dia as you can see in the image above when zoning into Promyvion-Dem.

## Spell Learning Notification

As mentioned earlier, during the situation when a player has a spell setup on their hotbar that they are high enough level to use, but have not learned the spell yet a small glowing scroll icon will appear in the top right corner of the spell. This disappears once learning that spell.
 
The below icon is dimmed because the player is high enough level to learn Cure 4 but has not learned yet, so there is a scroll icon on it. <br>
![Scroll Icon 1](images/readme/scroll1.png)

Furthermore, in reference to when spells have been setup in the same slots ("Spells Sharing Hotbar Slots" section) the glowing scroll icon will appear if any spell setup on that slot has not be learned yet (Any spell your currently high enough level to obtain as well). Regardless if it is a spell above or below the current spell being displayed. Once all spells setup on that slot have been learned the glowing scroll icon will disappear. For example, in the image above showing the four levels of protect setup on the same slot, if you are not high enough level to learn protect 3 and protect 4, but you have learned protect, and protect 2 then there will be no scroll icon displayed until you've reached the appropriate level for protect3.

The below icon is lit up because the player has learned Dia and this slot is currently useable to cast Dia. However, Dia 2 is setup on this slot as well and the player is high enough level to obtain it, but has not learned it yet, so there is a scroll icon on it. <br>
...
2. Inside your <name-of-character> folder create a general.lua and <job>.lua for each job you want to setup. Additionally, I have premade templates made for most jobs you can copy over to your play folder. The premade templates I have made in the Data/Technyze folder, and the RDM,WHM,THF,PLD are the more thorough jobs I have setup. The General.lua is the second page on the hotbar that is consistent across all jobs. Highly recommend downloading notepad++ for editing these files. 
...
```
4. Opening the game and logging into your character will generate a settings.xml based on the defaults.lua file (Do not edit the defaults.lua file). 
